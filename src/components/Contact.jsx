...
const Contact = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSubmitStatus({
        type: 'success',
        message: `Thanks ${formData.firstName}! Your message was sent successfully.`
      });
      setFormData({ firstName: '', lastName: '', email: '', message: '', permission: false });
    } catch (error) {
      console.error('EmailJS submission failed:', error);
      const errorMessage = error?.text?.replaceAll('&#x20;', ' ')
        || 'Your message could not be sent. Please try again.';
      setSubmitStatus({
        type: 'error',
        message: `EmailJS error: ${errorMessage}`
      });
    } finally {
  ...
  return (
    <section ref={ref} id="contact" className="bg-[#0b0b0b] w-full min-h-screen relative overflow-hidden flex items-end pt-32 pb-0 border-t border-white/10 select-none">
      ...
      {/* Huge Background Parallax Contact Watermark Text */}
      <motion.div 
        style={{ y }}
        ...
        >
          ...
          <div className="flex items-center justify-between mb-12 md:mb-16">
            ...
            <span className="text-xs font-mono text-white/40 tracking-wider hidden md:block">
              // FARHAN030719@GMAIL.COM • COIMBATORE
            </span>
          </div>
            ...
            <div className="flex flex-col md:flex-row gap-12 mt-4 pt-6 border-t border-white/10">
              ...
              <div className="flex-1 flex flex-col gap-8 text-xs text-white/50 font-light">
                <p className="leading-relaxed max-w-[400px]">
                  Based in Coimbatore and open to full-stack, Laravel, WordPress, and Shopify opportunities.
                </p>
                ...
                {submitStatus && (
                  <div
                    role="status"
                    aria-live="polite"
                    className={`flex items-start gap-3 border-l-2 px-4 py-3 font-mono text-xs leading-relaxed ${
                      submitStatus.type === 'success'
                        ? 'border-red-600 bg-red-600/10 text-white/80'
                        : 'border-red-500 bg-red-500/10 text-red-400'
                    }`}
                  >
                    <span className="mt-1 h-1.5 w-1.5 shrink-0 rounded-full bg-red-600"></span>
